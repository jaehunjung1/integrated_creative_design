...
with open(f"dataset/answers/dev_insurance.answer.pkl", 'rb') as f:
    answer_list = pickle.load(f)

with open(f"dataset/answers/dev_insurance.answer_index.pkl", 'rb') as f:
    answer_index_list = pickle.load(f)

with open(f"dataset/paragraphs/dev_insurance.paragraph.pkl", 'rb') as f:
    paragraph_list = pickle.load(f)

with open(f"dataset/paragraphs/dev_insurance.num_question.pkl", 'rb') as f:
    num_question_list = pickle.load(f)

with open(f"UnsupervisedMT/NMT/data/clozes/output_insurance.txt", 'r') as f:
    question_list = f.read().splitlines()
    question_list = [filter_mask(s) for s in question_list]

num_paragraph_list = [1] * len(paragraph_list)
...
with open(f"dataset/KorQuAD/augmented_insurance.json", 'w') as f:
