...
class EvaluatorMT(object):
    ...
    def inference_iterator(self):
        """
        Create a new iterator for a dataset.
        """
        dataset = self.data['inference']['cl']
        dataset.batch_size = 32
    ...
    def eval_inference(self):
        ...
        self.decoder.eval()
        params = self.params
        lang1_id = params.lang2id['cl']
        lang2_id = params.lang2id['qu']

        # hypothesis
        txt = []
        for batch in tqdm(self.inference_iterator()):
            ...
            # convert to text
            txt.extend(convert_to_text(sent2_, len2_, self.dico['qu'], lang2_id, self.params))

        output_file = os.path.join("./data/clozes", f"output{self.params.inference_num}.txt")
