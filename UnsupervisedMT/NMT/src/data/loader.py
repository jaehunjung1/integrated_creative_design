...
from .dataset import MonolingualDataset, ParallelDataset, InferenceDataset
...
def load_inference_data(params, data):
    assert len(params.inference_dataset) > 0

    data1 = load_binarized(params.inference_dataset, params)
    set_parameters(params, data1['dico'])

    if 'cl' not in data['dico']:
        data['dico']['cl'] = data1['dico']
    else:
        assert data['dico']['cl'] == data1['dico']

    inference_data = InferenceDataset(
        data1['sentences'], data1['positions'], data['dico']['cl'], params.lang2id['cl'], params
    )

    data['inference']['cl'] = inference_data
...
def load_data(params, mono_only=False):
    ...
    """
    data = {'dico': {}, 'mono': {}, 'para': {}, 'back': {}, 'inference': {}}
    ...
    if len(params.inference_dataset) > 0:
        # inference datasets
        load_inference_data(params, data)
